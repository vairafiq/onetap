...
const SettingContent = props => {
    ...
    const SettingContentData = [
        {
            ...
            content: [
                ...
                },
                {
                    label: "Exclude pages",
                    pro: true,
                    component: <Multiselect selectedValues={excludedPages} onSelect={handleExcludedPages} placeholder="Select Pages" options={oneTap_SettingsScriptData.wp_pages} displayValue="title" />
                },
                {
                    label: "Exclude in single post page",
                    component: <Multiselect selectedValues={excludedSingle} onSelect={handlEexcludedSingle} placeholder="Select Post Types" options={oneTap_SettingsScriptData.wp_post_types} displayValue="title" />
                },
                {
                    label: "Default user role",
                    pro: true,
                    component: <Multiselect selectedValues={defaultRole} onSelect={handlDefaultRole} placeholder="Select User Role" singleSelect={true} options={oneTap_SettingsScriptData.wp_roles} displayValue="title" />
                },
                {
                    label: "Auto signin",
                    component: <div className="exlac-vm-setting-has-info">
                ...
                },
                {
                    ...
                    pro: true,
                    component: <Multiselect selectedValues={context} onSelect={handlContext} placeholder="Select Context" singleSelect options={oneTap_SettingsScriptData.context} displayValue="title" />
                },
                {
    ...
    return (
        <SettingContentWrap className="onetap-settings-inner">
            <form action="" id="onetap_option_panel">
                {
                    SettingContentData.filter(item => item.key == "general")[0].content.map((settingItem, index) => {
                        let type = settingItem.pro ? 'onetap-settings__single onetap-pro-element' : 'onetap-settings__single';
                        return (
                            <div className={type} key={index}>
                                <h4 className="onetap-settings__single--label">{settingItem.label}
                                {
                                    settingItem.pro &&
                                    <sup className="onetap-pro-badge">Pro</sup>
                                }
                                </h4>
                                <div className="onetap-settings__single--element">
                                    {
